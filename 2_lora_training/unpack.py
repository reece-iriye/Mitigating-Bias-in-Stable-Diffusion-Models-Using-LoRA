...
directory = '/users/tdohm/work/Generated-LoRA-Input-Images-for-Mitigating-Bias'
...
df = pd.concat((pd.read_parquet(os.path.join(directory, pf)) for pf in parquet_files), ignore_index = True)

# Iterate Over DataFrame Rows
for index, row in df.iterrows():
    image_file_path = os.path.join(target_directory, f'{index + 1}.img')
